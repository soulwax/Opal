...
public class OpalGame : Game
{
    ...
    public OpalGame(string title, int width = 800, int height = 600)
    {
    ...
    protected override void LoadContent()
    {
        ...
        // Create placeholder textures - ensure they're the right size
        ...
        // NOW we can use _currentMap.TileSize for player positioning
        Vector2 playerStartPos = new Vector2(
            _currentMap.TileSize * 2 + _currentMap.TileSize / 2, // 2 tiles from left edge, centered in tile
            _currentMap.TileSize * 2 + _currentMap.TileSize / 2  // 2 tiles from top edge, centered in tile
        );

        // Initialize player with proper starting position
        _player = new Player(_playerTexture, playerStartPos);
